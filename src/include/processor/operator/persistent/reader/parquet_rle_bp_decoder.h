...
public:
    ...
    void GetBatch(uint8_t* values_target_ptr, uint32_t batch_size) {
        ...
        while (values_read < batch_size) {
            if (repeat_count_ > 0) {
                int repeat_batch = std::min<uint32_t>(
                ...
                uint32_t literal_batch = std::min<uint32_t>(
                    batch_size - values_read, static_cast<uint32_t>(literal_count_));
                uint32_t actual_read = ParquetDecodeUtils::BitUnpack<T>(
                    buffer_, bitpack_pos, values + values_read, literal_batch, bit_width_);
